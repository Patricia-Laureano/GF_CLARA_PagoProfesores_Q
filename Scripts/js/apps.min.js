...
var formValidation = function () {
    ...
    "use strict"; return {
        ...
        getMessage: function (msg) {
            ...
            var alert = "<div class=\"row\"><div id =\"alerta\" class=\"alert alert-warning fade in\"><span data-dismiss=\"alert\" class=\"close\"><img src=\"Content/images/icon-close.png\" style=\"margin-top:-7px\"></span><img src=\"Content/images/icon-warning.png\" class=\"pull-left\" style=\"margin-top:-7px\">";
...
function addEventsDireccion(dir) {
    ...
    document.getElementById(dir.idEstado).addEventListener("change", function () {
        ...
        dir.getValues();
        dir.getCiudades();
        //if (dir.Estado == 'Distrito Federal') {
        dir.getMunicipios();
        //}
    }, false);
...
var TabuladorList = function () {

    "use strict"; return {

        setTabuladorLists: function () {
            var model = { TabuladorList: $('#TabuladorList').val(), };
            $.ajax({
                type: "POST",
                dataType: 'json',
                cache: false,
                url: "/PA/getCampusPATab",
                data: model,
                success: function (data) {

                    if (TabuladorList.setTabuladorList_success != null && typeof TabuladorList.setTabuladorList_success == 'function')
                        TabuladorList.setTabuladorList_success(data);

                }, complete: handlerdataSedesTab

            });
        },

        setTabuladorList_success: function () {
            try {
                DataTable.init();
            } catch (e) { }
        },
...
function Pensionados(idElement) {
    ...
    this.getPensionados = function () {
        ...
        $.ajax({
            type: "GET",
            cache: false,
            url: "/PensionadosHelper/getPensionados/",
            data: "IdPersona=" + IdPersona,
            success: function (data) {
                $('#' + idElement).html(data);

            }//, complete: handlerdataPensionados
        });
    };
}

function Programas2(idElement) {
    this.idElement = idElement;  
    this.Escuela = "";
...
function CentrosCostos(idElement) {
    ...
    this.getCentrocostos = function () {
        ...
        $.ajax({
            ...
            data: "CampusVPDI=" + CampusVPDI + "&EscuelaCVE=" + EscuelaCVE + "&TipoFactura=" + TipoFactura + "&TipoPagoCVE=" + TipoPagoCVE + "&CVE_Programa=" + CVE_Programa,
            success: function (data) {
                $('#' + idElement).html(data);
...
function ConceptosdePago(idElement) {
    ...
    this.getConceptosdepago = function () {
        ...
        $.ajax({
            ...
            }, complete: handlerdatagetConceptosdePago
        });
    };
}
...
function FechasdePagoEsquema(idElement) {
    ...
    this.getFechasdepagoEsquema = function () {
        ...
        $.ajax({
            ...
            data: "EsquemaID=" + EsquemaID + "&FechaPago=" + FechaPago + "&PersonaID=" + PersonaID,
            success: function (data) {
                $('#' + idElement).html(data);
...
function Esquema(idElement) {
    ...
    this.getEsquema = function () {
        ...
        $.ajax({
            ...
            data: "Periodo=" + Periodo + "&CampusVPDI=" + Sede + /*"&Nivel=" + Nivel +*/ "&IdEsquema=" + IdEsquema,
            success: function (data) {
                $('#' + idElement).html(data);
            }
        });
...
function Direccion(idCP, idPais, idEstado, idCiudad, idMunicipio, idColonia) {
    ...
    this.getCiudades = function () {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                $('#' + idCiudad).html(data);
                $("#" + idCiudad + " option[value='" + Municipio + "']").prop('selected', true);
            },
        });
    };
    ...
    this.getMunicipios = function () {
        ...
        $.ajax({
            ...
            data: "Pais=" + Pais + "&Estado=" + Estado + "&Ciudad=" + (Ciudad == null ? "" : Ciudad) + "&Municipio=" + Municipio,
            success: function (data) {
...
function PartePeriodo(idElement) {
    ...
    this.init = function (obj) {
        ...
        //   addEventsTiposPago(dir);
        this.PartePeriodo = "";
        this.getPartePeriodo();
    };

    this.getPartePeriodo = function () {

        idElement = this.idElement;
        PartePeriodo = this.PartePeriodo;

        $.ajax({
            type: "GET",
            cache: false,
            url: "/PartePeriodo/getPartePeriodo/",
            data: "PartePeriodo=" + PartePeriodo,
            success: function (data) {
                $('#' + idElement).html(data);

            }, complete: handlerdataPartePeriodo
        });
    };
}

function TipoDeDocente(idElement) {
    ...
    this.getTipoDocente = function () {
        ...
        $.ajax({
            type: "GET",
            cache: false,
            url: "/TipoDocente/getTipoDocente/",
            //data: "PartePeriodo=" + PartePeriodo,
            success: function (data) {
                $('#' + idElement).val(data);

            }, complete: handlerdataTipoDocente
        });
    };
}

function TiposdePagosV(idElement) {
...
function Tabulador(idElement) {
    ...
    this.getTabulador = function () {
        ...
        $.ajax({
            type: "GET",
            cache: false,
            url: "/TabuladorX/ConsultarTabulador",
            data: "CampusVPDI=" + Sede,
            success: function (data) {
                $('#' + idElement).html(data);
            }
        });
...
var PB_timerProgressBar_Tick = function () {
    removeTimer();
    $.ajax({
        type: 'POST',
        url: '/GetProgress/Start',
        cache: false,
        data: 'id=' + PB_id_vdata,//añadido el parametro reset=true, por Alex Baca
...
//---------------------Function parse table tr

function RenderRowTable(_id_div, _id_table,html_table) {

                       var cadena = html_table.replace('<table', '|<table');
                        cadena = cadena.replace('</table>', '</table>|');

                        var arr = cadena.split('|');
                        var table = arr[1];

                        var tbody = arr[1].replace('<tbody>', '<tbody>|');
                        tbody = tbody.replace('</tbody>', '|</tbody>');
                        tbody = tbody.split('|');

                        var body = arr[0] + tbody[0] + tbody[2] + arr[2];


                        $("#" + _id_div).html(body);

                        var arrtbody = tbody[1].replace('</tr>', '</tr>|');
                        var arrtrbody = arrtbody.split('|');

                        //console.log(tbody[0]);
                        //console.log(arrtrbody[0]);

                        var n = "";
                        for (var i = 0; i < arrtrbody.length; i++) {
                            n = arrtrbody[i];
                            //$("#" + _id_table + " tbody").append(arrtrbody[i]);

                            $("#" + _id_table + " tbody").delay(100)
                                 .queue(function (nxt) {
                                     $(this).append(n);
                       ...
                       /* setTimeout(function () {
                            $('#DataTable-fixed').fixedHeaderTable({
                                altClass: 'odd',
                                footer: true,
                                fixedColumns: 4
                            });
...
function ayuda(url) {
    window.open("Help/"+url, "_blank", "toolbar=yes,scrollbars=yes,resizable=yes,top=10,left=500,width=400,height=400");
}
...
function domo() {
    ...
    $(document).bind('keydown', 'Ctrl+1', function (evt) { window.location.href = "/Esquemas"; return false; });
    $(document).bind('keydown', 'Ctrl+2', function (evt) { window.location.href = "/DatosPersonas"; return false; });
    $(document).bind('keydown', 'Ctrl+3', function (evt) { window.location.href = "/PA"; return false; });
    $(document).bind('keydown', 'Ctrl+4', function (evt) { window.location.href = "/NominaXCDC"; return false; });
    $(document).bind('keydown', 'Ctrl+5', function (evt) { window.location.href = "/EstadodeCuenta"; return false; });
    $(document).bind('keydown', 'Ctrl+6', function (evt) { window.location.href = "/RegistroContratos"; return false; });
    $(document).bind('keydown', 'Ctrl+7', function (evt) { window.location.href = "/GestiondePagos"; return false; });
