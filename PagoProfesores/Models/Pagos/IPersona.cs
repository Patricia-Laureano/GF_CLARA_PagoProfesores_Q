...
namespace PagoProfesores.Models.Pagos
{
    ...
    public partial class PAModel : SuperModel, IPersona
    {
        ...
        public long findPersona(bool TMP = false)
        {
            ...
            {
                sql = "";
                sql = "SELECT TOP 1 P.ID_PERSONA FROM PERSONAS P left JOIN PERSONAS_SEDES S ON S.ID_PERSONA = P.ID_PERSONA WHERE IDSIU = '" + IDSIU + "' ";
                ResultSet resPS = db.getTable(sql);
                if (resPS.Next())
                {
                    PK_PERSONA = resPS.GetLong("ID_PERSONA");
                    try
                    {
                        sql = "INSERT INTO PERSONAS_SEDES (ID_PERSONA, CVE_SEDE, CVE_TIPODEPAGO, FECHA_R, FECHA_M, USUARIO) " +
                            " VALUES (" + PK_PERSONA + ", '" + CVE_SEDE + "', '" + (TIPODEPAGOCODE == "RSCO" ? "RDI" : TIPODEPAGOCODE) + "', GETDATE(), GETDATE(), '" + sesion.pkUser.ToString() + "')";
        ...
        public bool editPersona(bool TMP = false)
        {
            ...
            if (res.Next())
            {
                ...
                TIPODEPAGOCODE = res.Get("CVE_TIPODEPAGO") == "RSCO" ? "RDI" : res.Get("CVE_TIPODEPAGO");
                RFC = res.Get("RFC");
                CURP = res.Get("CURP");
