...
namespace PagoProfesores.Models.Pagos
{
    public class EstadodeCuentaWebModel : SuperModel
    {
        ...
        public string DireccionFis_Domicilio { get; set; }
        ...
        public string UUID;
        public string sql;
        public string datosSqlNC;
        public string datosSqlCC;
        ...
        public string RAZONSOCIALCAMPUS { get; set; }


        public string MontoBancos { get; set; }
        public string MontoSubtotal { get; set; }

        public string Telefono_Soporte { get; set; }
        public string Campus { get; set; }
        public string Entidad_Estado_Pais_Sede { get; set; }
        public string Nombre_Responsable { get; set; }
        public string Sociedad_Mostrar { get; set; }
        public string Id_Esquema { get; set; }

        public string nacionalidad { get; set; }
        public string telefonoPersonal { get; set; }
        public string RFC_VALIDA { get; set; }
        public string CP_VALIDA { get; set; }
        public string existeUUID { get; set; }
        public string resultado { get; set; }

        public List<String> existeUUIDm = new List<String>();
        ...
        public bool VerificarCuenta()
        {
            ...
            sql = "SELECT ID_PERSONA " +
                ...
                " AND NOCUENTA_VALIDADO = 1" +
                " AND VALIDANOCUENTA = '" + NoCuenta + "'" +
                " AND VALIDACUENTACLABE = '" + CuentaClabe + "'";
        ...
        public void validaCuenta()
        {
            ...
            {
                datosSqlNC = "VALIDANOCUENTA ='" + NoCuenta + "',";
            }
            else
            {
                datosSqlNC = "";
            }
            if (!string.IsNullOrWhiteSpace(CuentaClabe))
            {
                datosSqlCC = "VALIDACUENTACLABE ='" + CuentaClabe + "',";
            }
            else
            {
                datosSqlCC = "";
            ...
            sql = "UPDATE PERSONAS SET "
                 ...
                 + datosSqlCC
                 + "NOCUENTA_VALIDADO = 1"
                 + ",FECHA_VALIDACUENTAS = GETDATE()"
                 + ", ID_USUARIO_VALIDACUENTAS='" + sesion.pkUser + "'"
                 + " WHERE IDSIU= '" + IDSIU + "' AND ID_PERSONA = '" + ID_PERSONA + "'";
        ...
        public void GetDatos()
        {
            ...
            if (res.Next())
            {
                ...
                Direccion = res.Get("DIRECCION_PAIS") + " " + res.Get("DIRECCION_ESTADO") + " <br/>Del/Mun." + res.Get("DIRECCION_ENTIDAD") + " Col." + res.Get("DIRECCION_COLONIA") + "<br/> Calle:" + res.Get("DIRECCION_CALLE") + " C.P." + res.Get("DIRECCION_CP");
                CorreoO365 = res.Get("CORREO365");
                if (res.Get("DATOSFISCALES").ToString() == "F")
                {
                    DireccionFis_Domicilio =
                      res.Get("FI_DIRECCION_CIUDAD") + ", " +
                      res.Get("FI_DIRECCION_CALLE") + ", " +
                      res.Get("FI_DIRECCION_COLONIA") + ", Del / Mun. " +
                      res.Get("FI_DIRECCION_ENTIDAD") + ", " +
                      res.Get("FI_DIRECCION_ESTADO") + ", C.P. " +
                      res.Get("FI_DIRECCION_CP");
                }
                else
                {
                    DireccionFis_Domicilio =
                    res.Get("RZ_DIRECCION_CIUDAD") + ", " +
                    res.Get("RZ_DIRECCION_CALLE") + ", " +
                    res.Get("RZ_DIRECCION_COLONIA") + ", " +
                    res.Get("RZ_DIRECCION_ENTIDAD") + ", " +
                    res.Get("RZ_DIRECCION_ESTADO") + ", C.P. " +
                    res.Get("RZ_DIRECCION_CP");
                }

                SedesAll = GetSedes(ID_PERSONA);

                GetPDFUUID(ID_PERSONA, Periodos, Sede);
            }
            else InitDatosP();
        }
        ...
        public string GetIDpersona(string pIdSiu)
        {
            string sedes = "";
            sql = "select top 1 ps.ID_PERSONA " +
                  "  from personas p inner join personas_sedes ps on ps.ID_PERSONA = p.ID_PERSONA " +
                  " where idsiu = '" + pIdSiu + "'";

            ResultSet res = db.getTable(sql);
            if (res.Next())
                sedes = res.Get("ID_PERSONA");
            return sedes;
        }
        ...
        public void GetDatosFiscales()
        {
            ...
            sql = "SELECT SO.* FROM SOCIEDADES SO, SEDES SE WHERE SE.CVE_SEDE LIKE '%" + Sede + "%' AND SO.CVE_SOCIEDAD =SE.SOCIEDAD_A_MOSTRAR";
        ...
        public string GetComunicados()
        {
            ...
            string periodoEdo = string.Empty;
            string esquema = string.Empty;
            string esquemaEdo = string.Empty;
            string hr = "<hr />";
            int pk1Edo = 0;
            ...
            // sql = " select id_esquema, periodo from estadodecuenta"
            //+ " where id_persona = " + ID_PERSONA
            //+ " order by id_estadodecuenta desc ";

            // ResultSet resEdo = db.getTable(sql);

            // if (resEdo.Count > 0)
            // {
            //     while (resEdo.Next())
            //     {
            //         periodoEdo = resEdo.Get("periodo");
            //         esquemaEdo = resEdo.Get("id_esquema");
            sql = "select * "
         + "  from vcomunicado"
         + " where cve_sede   = '" + Sede + "'"
         ...
         + " order by periodo desc, pkconceptopago asc";
            ...
            if (res.Count > 0)
            {
                ...
                {
                    periodo = res.Get("PERIODO");
                    //if (periodoEdo == periodo && esquemaEdo == res.Get("ID_ESQUEMA"))
                    //{
                    periodo = res.Get("PERIODO");
            ...
            if (esquema == "" && concepto == "")
            {
                paragraphs += "<p>No hay comunicados.</p>";
            }
            ...
            //    + "  from vcomunicado"
            //    + " where cve_sede   = '" + Sede + "'"
            //    + "   (id_persona = 0 or id_persona = " + ID_PERSONA + ")"
            //    + "   and periodo in (select periodo "
            //    + "                     from estadodecuenta"
            //    + "                    where cve_sede   = '" + Sede + "'"
            //    + "                      and id_persona = " + ID_PERSONA + ")"
            ...
            if (res.Count > 0)
            {
                ...
                while (res.Next())
                {
                    ...
                    table += "<tr>" +
                             ...
                             "<td> " + BANCOS + " </td>" +
                             "<td> " + Estado + " </td>" +
                              "<td><a href=\"/EstadodeCuentaWeb/DetallePago?_idestadocuenta=" + res.Get("ID_ESTADODECUENTA") + " \"> <img src='../../Content/images/detail.gif'> </a></td>" +
                             "</tr>";
                }
            }
            else
        ...
        public string GetPagosDashboardDepositados()
        {
            ...
            sql = "SELECT TOP 50 * FROM VESTADO_CUENTA WHERE CVE_SEDE = '" + Sede + "' AND ID_PERSONA =  " + ID_PERSONA + " AND PUBLICADO = 1 AND (FECHADEPOSITO <> NULL OR FECHADEPOSITO <> '')";
        ...
        public string GetDashboardContratos()
        {
            ...
            sql = "SELECT TOP 50 * FROM VENTREGA_CONTRATOS WHERE CVE_SEDE = '" + Sede + "' AND ID_PERSONA =  " + ID_PERSONA + " AND (FECHADECONTRATO IS NULL OR FECHADECONTRATO = '') AND PUBLICADO = 'SI'; "; //modifico <> ''
                ...
                while (res.Next())
                {
                    ...
                    table += "<li class=\"media media-sm\">" +
                              "<div class=\"media-body\">" +
                               btn_contrato + "<span style=\"font-size: 14px; font-weight: 500;\">FORMATO CONTRATO  /" + res.Get("CVE_SEDE") + " - " + res.Get("PERIODO") + " - (Monto: " + "<td>" + String.Format("{0:C}", MONTO) + "<td>" + ") " + Estado + "" +
        ...
        public string GetPagosPendientes()
        {
            ...
            sql = "SELECT TOP 50 * FROM VESTADO_CUENTA WHERE  CVE_SEDE = '" + Sede + "' AND ID_PERSONA =  " + ID_PERSONA + " AND PUBLICADO = 1 AND (FECHADEPOSITO IS NULL OR FECHADEPOSITO = '') AND PADRE = 0 ";
            ResultSet res = db.getTable(sql);
        ...
        public string GetPagosDepositados(HttpRequestBase Request)
        {
            ...
            sql =
                "SELECT TOP 50 * FROM VESTADO_CUENTA WHERE   CVE_SEDE = '" + Sede + "' AND  ID_PERSONA =  " + ID_PERSONA + " AND PUBLICADO = 1 AND (FECHADEPOSITO <> NULL OR FECHADEPOSITO <> '') " + condition + "";
        ...
        public string GetContratos(HttpRequestBase Request)
        {
            ...
            if (Request.Params["periodos"] != null && Request.Params["periodos"] != "")
            {
            ...
            sql = "SELECT TOP 50 * FROM VENTREGA_CONTRATOS WHERE CVE_SEDE = '" + Sede + "' AND  ID_PERSONA =  " + ID_PERSONA + " " + condition + " AND PUBLICADO = 'SI';";
            ...
            if (res.Count > 0)
            {
                while (res.Next())
                {
                    ...
                    {
                        Estado = "<label class=\"label label-warning\"><small>PENDIENTE</small></label><a href=\"javascript:void(0)\" onclick=\"formPage_Contratos.verContrato('" + res.Get("CVE_CONTRATO") +
                                    "','" + res.Get("CVE_SEDE") + "','" + res.Get("PERIODO") + "','" + res.Get("CVE_NIVEL") + "','" + res.Get("ID_ESQUEMA") + "','" + res.Get("IDSIU") + "');\" class=\"btn btn-xs btn-default m-r-5\"><i class=\"fa fa-print m-r-5\"></i> Imprimir</a>";
                    ...
                    else
                    {
                        ...
                        Estado = "<label class=\"label label-warning\"><small>ENTREGADO</small></label>";
                        //"<a href=\"javascript:void(0)\" onclick=\"formPage_Contratos.verContrato('" + res.Get("CVE_CONTRATO") +
                        //        "','" + res.Get("CVE_SEDE") + "','" + res.Get("PERIODO") + "','" + res.Get("CVE_NIVEL") + "','" + res.Get("ID_ESQUEMA") + "','" + res.Get("IDSIU") + "');\" class=\"btn btn-xs btn-default m-r-5\"><i class=\"fa fa-print m-r-5\"></i> Imprimir</a>";
                    ...
                    table += "<tr>" +
                            ...
                            "<td> " + res.Get("PERIODO") + " </td>" +
                            "<td> " + Estado + " </td>" +
                            "<td>" + String.Format("{0:dd/MM/yyyy}", res.GetDateTime("FECHAINICIO")) + "  </td>" +
                            "<td>" + String.Format("{0:dd/MM/yyyy}", res.GetDateTime("FECHAFIN")) + "  </td>" +
                            ...
                            "<td> " + string.Format("{0:C}", res.GetDecimal("MONTO")) + " </td>" +
                            "<td>" + (fecha_entrega != "" ? fecha_entrega : "No entregado") + "</td>" +
                            "</tr>";
                   ...
                   "SELECT TOP 24 * FROM PUBLICACION_RETENCIONES_MENSUALES WHERE CVE_SEDE = '" + res.Get("CVE_SEDE") + "' AND CVE_CICLO  = '" + res.Get("ANYODEPOSITO") + "' AND MES ='" + res.Get("MESDEPOSITO") + "'";
                    ...
                    if (publi_ret_mensual.Next())
                    {
                        ...
                        if (FECHA_ACTUAL >= fecha_publicacion_dt)// JUNIO(fs) NO ENERO SI(fs)
                        {
                            ...
                            "', '" + res.Get("MONTO") + "', '" + res.Get("MONTO_IVA") + "', '" + res.Get("MONTO_IVARET") + "', '" + res.Get("MONTO_ISRRET") + "', '" + res.Get("ID_ESQUEMA") + "');\"" +
                    ...
                    if (publi_ret_anual.Next())
                    {
                        ...
                        if (FECHA_ACTUAL >= fecha_publicacion_dt)
                        {
                            boton = "<a href=\"javascript:void(0)\" class='btn btn-xs btn-default m-r-5' onclick=\"formPage_RetencionesAnuales.verRetencion" +
                                ...
                                "', '" + res.Get("MONTO") + "', '" + res.Get("MONTO_IVA") + "', '" + res.Get("MONTO_IVARET") + "', '" + res.Get("MONTO_ISRRET") + "', '0');\"" +
                                " ><i class=\"fa fa-print m-r-5\"></i> Imprimir</a>";
        ...
        /*********************************/
        public string GetXMLUUID(string UUID)
        {
            sql = "SELECT UUID FROM ESTADODECUENTA WHERE UUID='" + UUID + "'"; //ET
        ...
        public void ValidarDatosFiscales()
        {
            ...
            string sql = "SELECT (p.[NOMBRES]+' '+p.[APELLIDOS]) AS DOCENTE ,p.RAZONSOCIAL ,p.[FI_DIRECCION_CP],DATOSFISCALES,ps.CVE_TIPODEPAGO, P.RZ_DIRECCION_CP FROM PERSONAS p " +
                    "INNER JOIN PERSONAS_SEDES ps ON p.ID_PERSONA=ps.ID_PERSONA WHERE IDSIU='" + IDSIU + "' AND PS.CVE_SEDE='" + Sede + "';";
            ...
            if (res.Next())
            {
                ...
                {
                    RAZONSOCIALDOCENTE = res.Get("DOCENTE").ToUpper();
                    FI_DIRECCION_CP = res.Get("FI_DIRECCION_CP");
                }
                else
                {
                    RAZONSOCIALDOCENTE = res.Get("RAZONSOCIAL").ToUpper();
                    FI_DIRECCION_CP = res.Get("RZ_DIRECCION_CP");
                }
            ...
            sql = "select top 1* from ESTADODECUENTA where ID_ESTADODECUENTA=" + ID_ESTADODECUENTA;
            ResultSet rescc = db.getTable(sql);
            if (rescc.Next())
            {
                Id_Esquema = rescc.Get("ID_ESQUEMA").ToUpper();
            }

             sql = "SELECT SO.* FROM SOCIEDADES SO, SEDES SE, ESQUEMASDEPAGO EQP WHERE SE.CVE_SEDE LIKE '%" + Sede + "%' AND SO.CVE_SOCIEDAD =EQP.CVE_SOCIEDAD AND EQP.ID_ESQUEMA=" + Id_Esquema;

            ResultSet resc = db.getTable(sql);
            if (resc.Next())
            {
                RAZONSOCIALCAMPUS = resc.Get("SOCIEDAD").ToUpper();
                RFC_VALIDA = resc.Get("RFC").ToUpper();
                CP_VALIDA = resc.Get("CP_VALIDA").ToUpper();
            }
            else
            {
                RAZONSOCIALCAMPUS = "";
            ...
            sql = "SELECT * FROM VESTADO_CUENTA WHERE IDSIU = '" + IDSIU + "' AND CVE_SEDE = '" + Sede + "' AND ID_ESTADODECUENTA =" + ID_ESTADODECUENTA + ";";
            ResultSet resced = db.getTable(sql);
            if (resced.Next())
            {
                MontoSubtotal = resced.Get("MONTO").ToUpper();
                MontoBancos = resced.Get("BANCOS").ToUpper();
            }

        }

        public void GetDatosSoporte()
        {
            sql = "SELECT * FROM SEDES WHERE CVE_SEDE = '" + Sede + "'";

            ResultSet res = db.getTable(sql);
            if (res.Next())
            {
                Campus = res.Get("SEDE") + ", " + res.Get("DIRECCION_CALLE") + " " + res.Get("DIRECCION_NUMERO") + ",";
                Entidad_Estado_Pais_Sede = res.Get("DIRECCION_ENTIDAD") + ", " + res.Get("DIRECCION_ESTADO") + ", " + res.Get("DIRECCION_PAIS") + ", C.P. " + res.Get("DIRECCION_CP");
                Nombre_Responsable = res.Get("NOMBRE_RESPONSABLE") + " ";
                Email_Sociedad = res.Get("CORREO_RESPONSABLE");
                Telefono_Soporte = res.Get("TELEFONO_RESPONSABLE") + " ";
            }
        }

        public object GetPDFUUID(string ID_PERSONA, string Periodos, string Sede)
        {
            sql = "SELECT UUID FROM ESTADODECUENTA WHERE ID_PERSONA=" + ID_PERSONA + " AND PERIODO='" + Periodos + "' AND CVE_SEDE='" + Sede + "'";
    ...
    public class FormatoXML
    {
        ...
        public string CP_RECEPTOR { get; set; } // ok
