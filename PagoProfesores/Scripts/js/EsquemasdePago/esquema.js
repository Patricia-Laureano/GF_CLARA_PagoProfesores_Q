...
$(function () {
    ...
    changeText.addEventListener("click", function () {
        ...
        if ($("#fechaf").val() != "") {
            document.getElementById("formbtnadd").style.display = "block";
...
var formPage = function () {
    ...
    "use strict"; return {
        ...
        edit: function (id) {
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    var sociedadesSelect = data.sociedades;
                    ...
                    $.ajax({
                        ...
                        dataType: 'json',
                        cache: false,
                        url: "/Esquemas/buscaSociedades/",
                        data: model,
                        success: function (data) {
                            var listado = "";
                            listado = "<select id='Cve_Sociedad' class='form-control'>";
                            listado += "<option value='0'>Seleccione una Sociedad</option>";
                            var socs = data.split('/');
                            for (var i = 0; i < socs.length; i++) {
                                if (socs[i].split(',')[0] == sociedadesSelect) {
                                    listado += "<option value='" + socs[i].split(',')[0] + "' selected >" + socs[i] + "</option>";
                                } else {
                                    listado += "<option value='" + socs[i].split(',')[0] + "'>" + socs[i] + "</option>";
                                }
                            ...
                            document.getElementById("formbtnadd").style.display = "none";
                            document.getElementById("formbtnadd2").style.display = "none";
                        }
                    });
                },
        ...
        save: function () { 
            ...
            var cvSociedad = document.getElementById("Cve_Sociedad").value;
            if (cvSociedad == 0) {
        ...
        add: function () {
            ...
            document.getElementById("divSociedades").innerHTML = "No se encontraron Sociedades dentro de la fecha de inicio y fecha de fin seleccionados."
                $.ajax({
                type: "POST",
                dataType: 'json',
                cache: false,
                ...
                data: model,
                    success: function (data) {
                        ...
                        if (data != null) {
                            ...
                            document.getElementById("divSociedades").innerHTML = listado;
                            document.getElementById("formbtnadd").style.display = "none";
                            document.getElementById("formbtnadd2").style.display = "block";
                            document.getElementById("formbtnadd2").disabled = false;
                        } else {
