...
var formPage_RetencionesAnualesPeriodo = function () {
    "use strict"; return {
        idEstadoCuenta: '',
        search: function () {
        },
        consultar: function () {
            DataTable_RetencionesAnualesPeriodo.init();
        },
        verRetencion: function (cve_retencion, cve_sede, mesi, mesf, anio, monto, monto_Iva, monto_IvaRet, montoIsrRet,id_esquema) {
            $.ajax({
                type: "GET",
                cache: false,
                url: '/ECW_RetencionesMensuales/SetRetencion',
                data: "cve_retencion=" + cve_retencion + "&cve_sede=" + cve_sede + "&mesi=" + mesi + "&mesf=" + mesf + "&anio=" + anio + "&monto=" + monto + "&monto_Iva=" + monto_Iva + "&monto_IvaRet=" + monto_IvaRet + "&montoIsrRet=" + montoIsrRet + "&id_esquema=" + id_esquema,
...
function filter() {
    var metodoControlle = "_ConstanciasRetencionAnualPeriodo";
    ...
    $.ajax({
        ...
        success: function (data) {
            $("#ConstanciasRetencionAnualPeriodo").html(data.ConstanciasRetencionAnualPeriodo);
        }, error: function (msg) {
        }
    });
}
...
var DataTable_RetencionesAnualesPeriodo = function () {
    var pag = 1;
    ...
    "use strict"; return {
        myName: 'DataTable_RetencionesAnualesPeriodo',
        idEstadoCuenta: '',
        ...
        exportExcel: function (table) {
            var data = 'Periodo=' + $('#Periodo').val()
                + '&Sede=' + $('#Sedes').val();
            window.location.href = '/ECW_RetencionesAnualesPeriodo/ExportExcel?' + data;
        },
        ...
        init: function () {
            ...
            $.ajax({
                type: "GET",
                cache: false,
                url: "/ECW_RetencionesAnualesPeriodo/CreateDataTable",
