...
var direccion1 = new Direccion('Direccion_CP', 'Direccion_Pais', 'Direccion_Estado', 'Direccion_Ciudad', 'Direccion_Entidad', 'Direccion_Colonia','CPValida');
var direccion2 = new Direccion('RL_Direccion_CP', 'RL_Direccion_Pais', 'RL_Direccion_Estado', 'RL_Direccion_Ciudad', 'RL_Direccion_Entidad', 'RL_Direccion_Colonia', 'Entidad_Federativa','Cve_LlaveMySuite_ID');
...
$(function () {
    $(window).load(function () {
        ...
        formValidation.notEmpty('CPValida', 'El campo "Código Postal" de la sseción Datos de Validación no debe de estar vacio');


        direccion1.init("direccion1");
        direccion2.init("direccion2");
        listadoEntidadFederativa(null);
        listadoLlavesMySuite(null);
    });
...
formPage = function () {
    ...
    "use strict"; return {
        ...
        clean: function () {    //limpia los campos y los deja listos para insertar uno nuevo
            ...
            var Cve_LlaveMySuite_ID = $("#Cve_LlaveMySuite_ID").val("");       //EDITAR
            ...
            var CPValida = $("#CPValida").val("");              //EDITAR
            var Cve_Entidad = $("#Cve_Entidad").val("");       //EDITAR
        ...
        edit: function (id) {
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    $("#CPValida").val(data.CPValida);                          //EDITAR                    
                    ...
                    listadoEntidadFederativa(data.Cve_Entidad);
                    listadoLlavesMySuite(data.Cve_LlaveMySuite_ID);
                }
            });
        },
        ...
        save: function () {
            var model = {
                ...
                Cve_Entidad: $("#Cve_Entidad").val(),
                CPValida: $("#CPValida").val(),                  //EDITAR
                ...
                Cve_LlaveMySuite_ID: $("#Cve_LlaveMySuite_ID").val(),
            }
...
function listadoEntidadFederativa(Cve_Entidad=null) {
    ...
    var dd = Cve_Entidad;
     ...
     $.ajax({
        ...
        dataType: 'json',
        cache: false,
        url: "/Sociedades/ListadoEntidadFederativa/",
        success: function (data) {
            if (data != null) {
                var listado = "";
                ...
                listado = "<select id='Cve_Entidad' class='form-control' >";
                var seleccionado = 0;
            ...
            document.getElementById("DivEntidad_Federativa").innerHTML = listado;
        }
    });
}

function listadoLlavesMySuite(Cve_LlaveMySuite) {

    var dd = Cve_LlaveMySuite;

    $.ajax({
        type: "POST",
        dataType: 'json',
        cache: false,
        url: "/Sociedades/ListadoLlavesMySuite/",
        success: function (data) {
            if (data != null) {
                var listado = "";
                var listadoLlavesMySuite = data;
                var listadoLlavesMySuite1 = listadoLlavesMySuite.split('/');
                var vacio = 0;

                listado = "<select id='Cve_LlaveMySuite_ID' class='form-control' >";
                var seleccionado = 0;
                for (var j = 0; j < listadoLlavesMySuite1.length; j++) {
                    if (dd != null && dd != 0) {
                        if (seleccionado != 1) {
                            if (listadoLlavesMySuite1[j].split('|')[0] == dd) {
                                listado += "<option selected value='" + listadoLlavesMySuite1[j].split('|')[0] + "'>" + listadoLlavesMySuite1[j].split('|')[1] + " / " + listadoLlavesMySuite1[j].split('|')[2] + " </option>";
                                seleccionado = 1;                         
                            }
                            listado += "<option value='" + listadoLlavesMySuite1[j].split('|')[0] + "'>" + listadoLlavesMySuite1[j].split('|')[1] + " / " + listadoLlavesMySuite1[j].split('|')[2] + "</option>";
                        }
                        seleccionado = 0;
                    } else {
                        if (vacio == 0) {
            ...
            document.getElementById("DivLlaves_MySuite").innerHTML = listado;
        }
    });
}